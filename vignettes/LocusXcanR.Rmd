...
date: "June 8, 2020, Updated: December 29, 2020"
...
```
LocusXcanR(twas_result, weight_tbl, study_name = "", pred_exp_corr, conditional_present = FALSE, multiple_tissues = FALSE, known_variants, known_gwas, db_genes, all_gwas, ld_gwas, ref_expr_name = "", head_details = "", method_details = "", primary_tissue, meta_present = FALSE, ideogram_present = FALSE, genome_build, cytoband_ds)
```
...
<ul>
  ...
  <li>`ideogram_present` logical, TRUE if an IdeogramTrack should be plotted, FALSE otherwise (optional, default is FALSE)
  <li>`genome_build` character, the genome build for the IdeogramTrack data (required if ideogram_present = TRUE). For example, `genome_build = "hg19"` for Genome Reference Consortium Human Build 37 (GRCh37).
  
  <li>`cytoband_ds` character, file path to the cytogenetic bands data set (required if ideogram_present = TRUE). See Ideogram plot section for more details on including an IdeogramTrack in the app.
</ul>
